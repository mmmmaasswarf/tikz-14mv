...
其中\codereplace{行数}和\codereplace{列数}分别表示棋盘的行数和列数.在\codereplace{内容}部分,你需要输入一个使用\verb|&|和\verb|\\|作为分隔符的字符串,关于\codereplace{图形设置},参加\ref{sec:图形设置}.\ \verb|&|用于分隔同一行每格之间的内容,而\verb|\\|用于开启下一行.此环境会将你输入的文本转换为对应元素依次置入题板,转换规则见下表.
